...
export function PostEditor() {
 ...
 try {
   ...
   font-medium text-gray-700">Title
   </label>
   <input
   id="post-title"
   value={title}
   onChange={(e) => { setTitle(e.target.value); setSta
  tus(STATES.EDITING); }}
   className="mt-1 block w-full rounded border-gray-30
  0 shadow-sm"
   aria-describedby={errorMessage ? "post-error" : undefined}
   />
   </div>
   <div>
   <label htmlFor="post-body" className="block text-sm f
  ont-medium text-gray-700">
   Body
   </label>
   <textarea
   id="post-body"
